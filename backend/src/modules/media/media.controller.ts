...
/** POST /media (multipart: file + kind + mime + peerId or channelId) -> { id, mime, size, width?, height? } */
export async function upload(req: Request, res: Response) {
  const target = uploadTargetSchema.parse(req.body);
  const { kind, mime } = mediaKindSchema.parse(req.body);
  const file = mediaFileSchema(kind, mime).parse(req.file);
